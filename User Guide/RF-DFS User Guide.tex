...
\end{tikzpicture}
\end{center}
\caption{RFI Shack PLC Wiring Diagram.}\label{fig:shackwiring}
\end{figure}
...
\end{tikzpicture}
\end{center}
\caption{Shack to DFS Serial Wiring Diagram.}\label{fig:serialwiring}
\end{figure}
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
...
The best-performing design as of date uses Mueller BU-1420761881 connectors with the footprint shown in Figure~\ref{fig:sma}. This footprint is also present in the repository, but the layer 2 ground cutout needs to be generated manually for each connector. The layer 1 ground cutout may also perform better with its right angles smoothed although the dimensions for this are not strict. A grounded coplanar waveguide (Shown in Figure~\ref{fig:gcpw}) extends in a straight line from the SMA center conductor to the MMIC input pin. The dimensions were chosen as a compromise between easy of soldering components, where a larger trace width is better, and impedance matching the trace to MMIC pins, where a smaller trace width is better. As a result, only 0402 components will fit directly on the trace and SMA connectors with a center pin larger than 10 mils will likely incur problems as well.
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}[x=1cm, y=1cm, z=0.6cm]
      \foreach \y in {0, 1, 2, 3} {\coordinate(ne\y) at (4.5, \y, 5);}
      \begin{scope}[canvas is xz plane at y=0]
        \draw(0, 0) node[anchor=north west]{Layer 4};
        \draw(0, 0) [fill=pcbother]rectangle (ne0);
      \end{scope}
      \begin{scope}[canvas is xz plane at y=1]
        \draw(0, 0) node[anchor=north west, fill=white]{Layer 3};
        \draw(0, 0) [fill=pcbother]rectangle (ne1);
      \end{scope}
      \begin{scope}[canvas is xz plane at y=2]
        \draw(0, 0) node[anchor=north west, fill=white]{Layer 2};
      ...
      \begin{scope}[canvas is xz plane at y=3]
        \draw(0, 0) node[anchor=north west, fill=white]{Layer 1};
        ...
        \draw(4, 2.5) node[fill=pcbcopper]{1.41};
